#Sistema que permite o cadastramento de uma frota para uma empresa de logística
#e faça escolha do veículo que atenderá a demanda.

#Funcionalidades: cadastro de veículo (insere, altera e consulta)
...
def insere_veiculo(lista):
    ...
    capacidade = float(input("Informe a capacidade em TON: "))
    lista.append(tipo)
    lista.append(placa)
    lista.append(modelo)
...
while op != 5:
    if op == 1:
        ...
        if pos != -1:
            ...
            print(f"Não existe veículo com a placa informada: {placa}")
    elif op == 3:
        placa = input("Placa : ")
        pos = consulta_veiculo(placa, veiculos)
        pos = pos - 1
